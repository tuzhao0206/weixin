<template>
  <ex-view :class="$style.style">
    <ex-header title="选项卡 (参考有赞)">
      <ex-menu class="text-gray" @click="$router.back();"> <i class="icon">&#xe60e;</i> </ex-menu>
      <ex-title />
    </ex-header>
    <ex-content>
      <ex-tabview>
        <ex-tab v-for="(item, idx) in labels(3)" :key="idx" :title="item"> {{ item }} </ex-tab>
      </ex-tabview>

      <div class="divider" ui-mode="25%">超过四个标签滚动</div>

      <ex-tabview>
        <ex-tab v-for="(item, idx) in labels(10)" :key="idx" :title="item"> {{ item }} </ex-tab>
      </ex-tabview>
      ...
      <ex-tabview v-model="index">
        <ex-tab v-for="(item, idx) in labels(4)" :key="idx" :title="item"> 当前TAB为: {{ index }} </ex-tab>
      </ex-tabview>
    </ex-content>
  </ex-view>
</template>
